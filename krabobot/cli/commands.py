...
from krabobot import __logo__, __version__
from krabobot.cli.stream import StreamRenderer, ThinkingSpinner
from krabobot.config.paths import get_workspace_path, is_default_workspace
from krabobot.config.schema import Config
from krabobot.utils.helpers import sync_workspace_templates

app = typer.Typer(
    name="krabobot",
    context_settings={"help_option_names": ["-h", "--help"]},
    help=f"{__logo__} krabobot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from krabobot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} krabobot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} krabobot[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} krabobot v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """krabobot - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    if wizard:
        from krabobot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'krabobot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
...
def _onboard_channels(config_path: Path) -> None:
    """Inject default config for all built-in discovered channels."""
    import json

    from krabobot.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from krabobot.config.loader import load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from krabobot.config.paths import get_cron_dir
...
def gateway(
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from krabobot.utils.evaluator import evaluate_response
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show krabobot runtime logs during chat"),
):
...
users_app = typer.Typer(help="Manage user identity links")
app.add_typer(users_app, name="users")


def _build_user_resolver(config: Config):
    from krabobot.users import UserResolver

    multi_user_cfg = config.tools.multi_user
    return UserResolver(
        config.workspace_path / "identity",
        code_ttl_seconds=multi_user_cfg.link_code_ttl_seconds,
        code_attempt_limit=multi_user_cfg.link_code_attempt_limit,
    )


@users_app.command("link")
def users_link(
    user_id: str = typer.Argument(..., help="Internal user ID"),
    channel: str = typer.Argument(..., help="Channel name (e.g. telegram, vk, email)"),
    sender_id: str = typer.Argument(..., help="Channel sender/account ID"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory override"),
):
    """Manually link a channel account to an internal user ID."""
    runtime_config = _load_runtime_config(config, workspace)
    resolver = _build_user_resolver(runtime_config)
    asyncio.run(resolver.link_account(user_id=user_id, channel=channel, sender_id=sender_id))
    console.print(
        f"[green]✓[/green] Linked [cyan]{channel}:{sender_id}[/cyan] -> [bold]{user_id}[/bold]"
    )
...
def users_list(
    ...
    """List known internal users and linked channel accounts."""
    import json

    runtime_config = _load_runtime_config(config, workspace)
    identity_file = runtime_config.workspace_path / "identity" / "user_links.json"
    if not identity_file.exists():
        console.print("[yellow]No user links found yet.[/yellow]")
        return

    try:
        payload = json.loads(identity_file.read_text(encoding="utf-8"))
    except Exception as e:
        console.print(f"[red]Failed to read identity store:[/red] {e}")
        raise typer.Exit(1)
    ...
    for account_key, uid in accounts.items():
        by_user.setdefault(str(uid), []).append(str(account_key))

    table = Table(title="Linked Users")
    table.add_column("User ID", style="cyan")
    table.add_column("Accounts", style="magenta")
    table.add_column("Count", justify="right", style="green")

    for uid, linked_accounts in sorted(by_user.items()):
        linked_accounts.sort()
        table.add_row(uid, "\n".join(linked_accounts), str(len(linked_accounts)))
...
def status():
    ...
    console.print(f"{__logo__} krabobot Status\n")
