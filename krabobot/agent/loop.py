...
class _LoopHook(AgentHook):
    ...
    async def on_stream(self, context: AgentHookContext, delta: str) -> None:
        from krabobot.utils.helpers import strip_think
...
class AgentLoop:
    ...
    async def _send_tool_message(self, msg: OutboundMessage) -> None:
        """Validate target availability before enqueuing a tool-sent message.
        ...
        section = getattr(channels_cfg, channel, None) if channels_cfg is not None else None
        if section is None:
            raise RuntimeError(f"Channel '{channel}' is not configured.")

        if isinstance(section, dict):
            enabled = bool(section.get("enabled", False))
            consent = bool(section.get("consentGranted", section.get("consent_granted", False)))
        else:
            enabled = bool(getattr(section, "enabled", False))
            consent = bool(getattr(section, "consent_granted", False))

        if not enabled:
            raise RuntimeError(f"Channel '{channel}' is disabled in config.")
        if channel == "email" and not consent:
            raise RuntimeError("Email delivery is blocked: set channels.email.consentGranted=true.")
    ...
    async def _connect_mcp(self) -> None:
        ...
        from krabobot.agent.tools.mcp import connect_mcp_servers
        try:
