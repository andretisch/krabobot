...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("krabobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("krabobot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("krabobot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    if get_cron_dir is not None:
        monkeypatch.setattr("krabobot.config.paths.get_cron_dir", get_cron_dir)
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from krabobot.cli.commands import _migrate_cron_store
    ...
    with patch("krabobot.config.paths.get_cron_dir", return_value=legacy_dir):
